...
class _WaveWidgetState extends State<WaveWidget> with TickerProviderStateMixin {
    ...
    if (widget.config.colorMode == ColorMode.custom) {
      ...
      if (!widget.isLoop) {
        _endAnimationTimer = Timer(Duration(milliseconds: widget.duration), () {
          for (var waveController in _waveControllers) {
            waveController.stop();
          }
        });
      }
  ...
  List<Widget> _buildPaints() {
    var paints = <Widget>[];
    if (widget.config.colorMode == ColorMode.custom) {
      var _colors = (widget.config as CustomConfig).colors;
      var _gradients = (widget.config as CustomConfig).gradients;
      var begin = (widget.config as CustomConfig).gradientBegin;
      var end = (widget.config as CustomConfig).gradientEnd;
