...
class LikeButtonState extends State<LikeButton> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    var likeCountWidget = _getLikeCountWidget();
    if (widget.countDecoration != null) {
      likeCountWidget =
          widget.countDecoration?.call(likeCountWidget, _likeCount) ??
              likeCountWidget;
    }
    ...
    var children = <Widget>[
      AnimatedBuilder(
        ...
        builder: (BuildContext c, Widget? w) {
          final likeWidget = widget.likeBuilder?.call(_isLiked) ??
  ...
  Widget _getLikeCountWidget() {
    ...
    final likeCount = _likeCount.toString();
    final preLikeCount = _preLikeCount.toString();
    ...
    final allChange = preLikeCount.length != likeCount.length || didIndex == 0;
    ...
    if (widget.likeCountAnimationType == LikeCountAnimationType.none ||
      ...
      result =
        ...
        !allChange) {
      final samePart = likeCount.substring(0, didIndex);
      final preText = preLikeCount.substring(didIndex, preLikeCount.length);
      final text = likeCount.substring(didIndex, likeCount.length);
      final preSameWidget =
          _createLikeCountWidget(_preLikeCount, !_isLiked, samePart);
      final currentSameWidget =
          _createLikeCountWidget(_likeCount, _isLiked, samePart);
      final preWidget =
          _createLikeCountWidget(_preLikeCount, !_isLiked, preText);
      final currentWidget = _createLikeCountWidget(_likeCount, _isLiked, text);
