...
class _SlidingListActionState extends State<SlidingListAction>
  ...
  Future<List<Conversation>> cardList() async {
    String responseJson = await rootBundle.loadString('assets/dummy.json');
    List cards = json.decode(responseJson);
    List<Conversation> listCard =
        cards.map((card) => Conversation.fromJson(card)).toList();
    return listCard;
  }
